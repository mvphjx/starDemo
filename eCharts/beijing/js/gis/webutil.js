...
var WebUtil =
{
    ...
    detectBrowser:function () {
		...
		var m = ua.match(re);
        browser.browser = m[1].replace(/Version/, "'Safari");
        browser.version = m[2];
		return browser;
    },
    version : function(){
	...
	{
		//var clientHeight = $(document.documentElement).height();  不兼容ie11  2017年12月11日 韩健祥
	...
	利用控件获取机器信息
	@link ComputerInfo
	{"computerName":null,"ips":null,"macs":null,"serviceNo":null}
	 */
    getComputerInfo:function()
    {
		var ocx = document.getElementById("ComputerInfoOcx");//需要引用 ocx插件
		var info = {"computerName":null,"ips":null,"macs":null,"serviceNo":null};
        try {
            if(ocx&&ocx.HS_OCX_GetComputerInfo){
                var ocxinfo = ocx.HS_OCX_GetComputerInfo();
                ocxinfo = this.string2Json(ocxinfo);
                info.computerName=ocxinfo["ComputerName"];
                info.ips=ocxinfo["IP"];
                info.serviceNo=ocxinfo["ServiceNo"];
                info.macs=null;
            }
		}catch (e){}
        return info;
    ...
    //180101 ->"18:01:01"
    string2TimeString:function(string){
        if(this.isEmpty(string)||string.length!==6){
            return string;
        }
        var pattern = /(\d{2})(\d{2})(\d{2})/;
        string = string.replace(pattern, '$1:$2:$3');
        return string;
    },
    /**
	 * 移除 指定数组元素
    ...
    removeAll: function (arr1, arr2) {
        ...
        if (this.isArray(arr1) && this.isArray(arr2)) {
            for (var i1 = 0; i1 < arr1.length; i1++) {
                var temp1 = arr1[i1];
                if ($.inArray(temp1, arr2) === -1) {
                    result.push(temp1);
                }
            }
        }else if(this.isArray(arr1)){
            result = arr1;
		}
        return result;
    },
