...
SwitchControl.prototype.init = function()
{
	...
	this.initDownList(this.layoutId);
...
SwitchControl.prototype.initTree = function()
{
	...
	{
		treePanel[0].contentWindow.initTree(settings, this.treeData);
		this.refreshTree();
	}
	else
	{
...
SwitchControl.prototype.initDownList = function(listId)
{
	$btyFrame = $("#" + listId + "Frame");
	var $head = $btyFrame.contents().find("head");
	var $body = $btyFrame.contents().find("body");
	...
	var before=this.IEVersion = 8?"http://"+window.location.host:"";
	var cssUrl =before+WebVar.VarPath + "/css/abis/ui/switchcontrol/drop_down_list.css";
	$css.attr("href", cssUrl);
	$head.append($css);

	var cssName = null;
	if (listId == this.imgIdId || listId == this.imgComboId)
	{
		cssName = "imgIdList";
	}
	...
	switch (listId)
	{
		...
		case this.fgpId:
			...
			break;
		case this.imgIdId:
		case this.imgComboId:
			clsName = "imgIdHeight";
			break;
		case this.gidId:
		case this.groupComboId:
			clsName = "gidHeight";
			break;
		case this.cidId:
			clsName = "cidHeight";
...
SwitchControl.prototype.initBtyList = function()
{
	...
	var $up = $node.find("div:eq(0)").find("span:eq(0)");
	var $downList = $("#" + this.id + "BtyFrame").contents().find("div");
	...
	if (this.fiveList != null)
	{
		...
		{
			five = this.fiveList[i];
			this.addItem($downList, five.btyText, five.bty, this, this.btyId);
			if (five.bty == this.bty)
			{
	...
	if (firstText == null)
	{
		if (this.fiveList != null && this.fiveList.length > 0)
		{
			this.bty = this.fiveList[0].bty;
			firstText = this.fiveList[0].btyText;
		}
		else
		{
			this.bty = null;
