...
var searchBoxObj = {
    ...
    doSearch: function () {//搜索
        ...
        $(".theme_list span[name='"+ layerId +"']").addClass("on");
        $(".loading").next().hide();
        $(".loading").show();

        var cql_filter = "";
        if (text) {
            cql_filter = "TITLE like '%" + text + "%'";
        }
        $('div.left_box').show();
        //数据铺到地图并回调渲染列表
        mapTool.showGeoServerData(layerId, typeName, cql_filter, ulDetailsObj.renderData);
    },
...
var ulDetailsObj = {
    ...
    content: {//页面列表等内容
        camera: function () {//摄像头列表
            for (var i = 0; i < ulDetailsObj.data.length; i++) {
                ...
                var coordinates = elem.getGeometry().getCoordinates();
                var html =
    ...
    renderData: function (_dataType, result) {//渲染列表
        ulDetailsObj.data = result;
        $("#searchCount").text(result.length);
        $(".loading").next().show();
        $(".loading").hide();
...
var cameraOrgTree = {
    ...
    clickNode: function (event, treeId, treeNode) {
        if (treeNode) {
            ...
            if (type == 'org') { //节点类型为机构，将该机构摄像头数据加载到地图
                ...
                mapTool.showGeoServerData('camera', 'e_camera', cql_filter);
    ...
    loadOrgCamerasToTree: function (event, treeId, treeNode) {
        if (treeNode) {
            ...
            mapTool.showGeoServerData('camera', 'e_camera', cql_filter, function (layerId, result) {
