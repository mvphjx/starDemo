...
function FigureCheck(qryId,pageNumLan)
{
    this.initParam();
    this.qryId 			= qryId;
...
FigureCheck.prototype.isFillScreen = false;
FigureCheck.prototype.loadFirstTT = true;//第一次加载TT 如果缺指位需要特殊处理
FigureCheck.prototype.candResults = {};//候选列表  认定结果缓存，防止翻页之后丢失 id：value
FigureCheck.prototype.firstPageFirstLoadDoNotNeedLoad = false;//优化避免重复加载
FigureCheck.prototype.init = function()
{
    this.defaultLayout();
    this.initButton();
    this.initCheckOcx();
...
FigureCheck.prototype.fs = function()
{
...
/** 切换屏幕显示模式 */
FigureCheck.prototype.switchScreenMode = function()
{
...
FigureCheck.prototype.initParam = function()
{
    this.candTable 		= null;
...
FigureCheck.prototype.initButton = function()
{
    var nThis = this;
 ...
 * 退出全屏模式
 */
FigureCheck.prototype.exitFullScreen = function()
{
    ...
    $(".FgpFrame").animate({"width":"100%","height":"50%"},150);
    $("#SwitcherPanel").animate({"width":"501px"});
    $(".LeftFgpLayout").animate({"width":"501px"});
    $(".CheckLayout").animate({"width":"501px"});

    var hh = $(".BodyC").outerWidth(true);

    var rigW = hh-leftW-501-5;

    $(".CenterLayout").animate({"marginLeft":(leftW + 1) + "px","marginRight":(rigW + 1)+ "px"},100);
    $(".WebMenu").show();

    $(".RightLayout").animate({"width":rigW+"px"});
 ...
 * 进入全屏模式
 */
FigureCheck.prototype.fullScreen = function()
{
    hiddenSlider();
...
//新加
    $(".CheckLayout").animate({"width":WebUtil.getClientWidth()+"px"},150);
    $(".SwitcherPanel").animate({"width":WebUtil.getClientWidth()+"px"},150);
    $(".LeftFgpLayout").animate({"width":WebUtil.getClientWidth()+"px"},150);
...
/** 自动调整控件尺寸 */
FigureCheck.prototype.defaultLayout = function()
{
    ...
    $(".LeftLayout").animate({"height":height+"px"},150);
    $("#QryCandTable").animate({"height":(height-30)+"px"},150);
    $("#CardTab").animate({"height":(WebUtil.getContentHeight()/2)+"px"},150);
    $("#ImgProView").animate({"height":(WebUtil.getContentHeight()/2)+"px"},150);
    $("#MatchTaskInfo").height((WebUtil.getContentHeight()/2)+"px");
    ...
    if(height > rightTop)
    {
        ...
        var candInfoH 	= $("#SrcCard").outerHeight(true);
        var imgProH 	= $("#QryTask").outerHeight(true);
        var newH 		= height - candInfoH  - imgProH - titleH - rMaginBorder;
        //console.log("ImgProcess="+imgProH+newH-titleH)
        var imgProcessH = $("#ImgProcess").outerHeight(true);
        ...
        var candCardH = imgProH+newH+titleH+srcCardInfoH;
        //$("#CandCard").height(candCardH);
        //console.log("ocxHeight"+(ocxHeight+candInfoH)/3);
        ocx.height(candCardH/2-35);
        //console.log("高度"+candCardH);
        //$(".RightLayout").animate({"width":1100  + "px"});
		/*$("#SwitcherPanel").animate({"width":"501px"});
		 $(".LeftFgpLayout").animate({"width":"501px"});
		 $(".CheckLayout").animate({"width":"501px"});*/
...
/** 完成认定 之前进行校验 无候选提示 */
FigureCheck.prototype.beforeFinishCheck = function(){
    var _this = this;
 ...
 * 重置页面
 */
FigureCheck.prototype.reSet = function()
{
    // 如果不清空页面则将候选列表右键菜单更新
...
/** 打开比对任务 */
FigureCheck.prototype.openMatch = function()
{
 ...
 * rows表示表格选中的行
 */
FigureCheck.prototype.openMatchTask = function(rows)
{
...
FigureCheck.prototype.open = function(qryId)
{
    var nThis = this;
...
// 切换比对任务
FigureCheck.prototype.switchMatchTask = function(matchInfo)
{
    ...
    this.candDataTable();
    // 更新候选列表菜单文本
    //ll this.updateMenuTxt();

    // 清空候选 认定结果缓存
    this.candResults={};
}

FigureCheck.prototype.updateSrcInfo = function(matchInfo)
{
    var qryObj	= matchInfo.qryObj;
    var taskInfo = qryObj.taskInfo;
...
FigureCheck.prototype.updateTPCardInfo = function(info,tpInfo)
{
...
FigureCheck.prototype.updateLPCardInfo = function(info,lpInfo)
{
...
FigureCheck.prototype.updateMatchTaskInfo = function(taskInfo)
{
    this.clearMatchTaskInfo();
...
/** 更新弹出菜单项文本 */
FigureCheck.prototype.updateMenuTxt = function()
{
...
/** 根据比对状态对象生成相应文本 */
FigureCheck.prototype.getStatusFlowTxt = function()
{
...
//切换候选
FigureCheck.prototype.switchCand = function(row)
{
...
FigureCheck.prototype.updateCandInfo = function(cardId)
{
...
FigureCheck.prototype.updateCandCache = function()
{
 ...
 *
 */
FigureCheck.prototype.syncSrcBfvc = function(row)
{
    var qryObj 		= this.matchInfo.qryObj;
    var qryType 	= qryObj.taskInfo.qryType;
...
FigureCheck.prototype.syncSrcAndDstBfvc = function(type)
{
...
FigureCheck.prototype.initCandTable = function()
{
    var  nThis = this;
    ...
    //创建人像候选列表表格
    FigureCheck.prototype.candDataTable = function (){
        var cols=new Array();
        cols.push("ID");
        cols.push("NAME");
        ...
        var tblParam =
            {
                tblId	: "QryCandTable",
                tblName		: "tp_face1_view",
                cols		: cols,
                pageSize    : 15,
                url			: WebVar.VarPath + "/tp/tpface/list",
                order		: "ID",
        ...
        var faceTable  = new TableCustomizeMgr(tblParam);
        //this.loadFirst();
    }
    ...
    function searchFinish(data)
    {
        ...
        for(var i=1;i<=resData.length/6+1;i++)
        {
            $("#tr"+i).show();
        }
        for(var i=0;i<resData.length;i++)
        {
            var person=resData[i];
            if(person!=null&&person!=undefined)
            {
                var personid = person["data"]["ID"];
                var name = person["data"]["NAME"];
                var cardnum = person["data"]["CARD_NUM"];
                var birtdate = person["data"]["BIRTH_DATE"];
                var index=i+1;
                var id="face"+index
                var faceitem=new TPFaceImage(id,personid,name,cardnum,birtdate,"n");
                facelist.push(faceitem);
            }
        }
    ...
    function clearImg()
    {
        $(".facetable tr").each(function(){$(this).hide();});
        for(var i=1;i<25;i++)
        {
            $("#face"+i).html("");
        }
...
FigureCheck.prototype.initInfoUI = function()
{
...
FigureCheck.prototype.updateInfoUI = function(qryType)
{
...
FigureCheck.prototype.clearMatchTaskInfo = function()
{
    //比对信息
    $("#QryNum").html("");
    $("#QryType").html("");
 ...
 * 清空信息
 */
FigureCheck.prototype.clearInfo = function()
{
 ...
 * 根据比对ID检索比对候选
 */
FigureCheck.prototype.searchCandInfo = function()
{
    if(this.matchInfo == null)return;
...
FigureCheck.prototype.loadFirst = function()
{
    if(this.matchInfo == null)return;
    ...
    // 获取候选卡片信息
    //this.updateCandInfo(this.dstCardId);
   /* alert("this.dstCardId="+this.dstCardId);
    ttText(this.dstCardId);*/
}
function ttText(tpCardId) {
    var setting =
        {
            onClick		:click,
            onDblClick	:dblClick
        }
    this.layoutMgr = new TPLayoutMgr("QryCandTable",tpCardId,setting);
}
...
/** 初始化认定控件 */
FigureCheck.prototype.initCheckOcx = function()
{
    //初始化认定控件参数
    this.imgOcx1 = document.getElementById("CheckImageOcx1");
    this.imgOcx = document.getElementById("CheckImageOcx");
    //this.imgOcx1.HS_OCX_SetBackColor(WebVar.ImgOcxBg);
    //this.imgOcx.HS_OCX_SetBackColor(WebVar.ImgOcxBg);
}
FigureCheck.prototype.cacheTP = function(cardId,bty,vid, fgp, cid, gid, dataType, imgType, loc){//缓存图像
    if(cardId==null){
        return;
    }
    var obj 		= {};
...
FigureCheck.prototype.searchTP = function(cardId,bty,vid, fgp, cid, gid, dataType, imgType, loc,isCand)
{
    if(cardId== null)return;
    ...
    jQuery.ajax(
        {
            ...
            success : function(data)
            {
                if(data != null)
                {
                    ...
                    var imgOcx = document.getElementById("CheckImageOcx");
                    var imgOcx1 = document.getElementById("CheckImageOcx1");
                    if(isCand){
                        //$("#tt").append("<img  src='data:image/jepg;base64,"+img+ "'"+" width='150px' height='200px'/>");
                        imgOcx1.HS_OCX_SetImageMnt(img, "", mnt, "");
                    }else{
            ...
            error : function(e)
            {
                DialogUtil.openComfirmDialogForOcx(AbisMessageResource.Alert['SearchError']);
...
//缓存图像信息
FigureCheck.prototype.cacheLP = function(cardId, bty, cid, dataType, imgType, gid, loc){
    if(cardId == null)return;
    var obj 	= {};
    obj.bty 	= bty;
    obj.cid 	= cid;
...
FigureCheck.prototype.searchLP = function(cardId, bty, cid, dataType, imgType, gid, loc,isCand)
{
    if(cardId== null)return;
    //if(WebUtil.isNull(this.layoutMgr) ) return;
    var obj 	= {};
	...
	/*if(cardId == null)return;
	 ...
	 if(nThis.imgOcx1 != null)
	 {
	 if (imgLob != null && !WebUtil.isEmpty(imgLob.item))
	 {
	 var lob 	= imgLob.lob;
	 var item 	= imgLob.item;
	 nThis.imgOcx1.HS_OCX_AppendCheckLPStrLob(ceId, cardId, lob.id, item.itemIndex, bty, ABISCode.LobDataType.IMG, lob.gid, cid, item.data,loc);
	 }
	 if (mntLob != null && !WebUtil.isEmpty(mntLob.item))
	 {
	 var lob 	= mntLob.lob;
	 var item	= mntLob.item;
	 nThis.imgOcx1.HS_OCX_AppendCheckLPStrLob(ceId, cardId, lob.id, item.itemIndex, bty, ABISCode.LobDataType.MNT, lob.gid, cid, item.data,loc);
	 }
	 nThis.imgOcx1.HS_OCX_SelectImageMnt(cardId, 0, bty, 0, 0, obj.imgType, obj.gid, cid);
	 nThis.imgOcx1.HS_OCX_DoDigitMap(0);
	 }
	 }
	 },
	 error : function(e,status,error)
...
FigureCheck.prototype.selectedSrcFgp = function(id, bty, vid, fgp, cid, imgType, mntId)
{
...
/** 切换候选卡指位*/
FigureCheck.prototype.selectedDstFgp = function(id, bty, vid, fgp, cid, imgType, mntId)
{
    this.searchBfvc(this.dstCardId, bty, vid, fgp, cid, imgType, mntId, true);
}

FigureCheck.prototype.searchBfvc = function(cardId, bty, vid, fgp, cid, imgType, mntId,isCand)
{
    if(cardId==null){
        return;
    }
    if (bty == null || vid == null || fgp == null || cid == null){
		...
		/* this.imgOcx.HS_OCX_SelectImageMnt(cardId, 1, bty, vid, fgp, cid, imgType, mntId);
		 ...
		 this.imgOcx1.HS_OCX_SelectImageMnt(cardId, 1, bty, vid, fgp, cid, imgType, mntId);
		 this.imgOcx1.HS_OCX_DoDigitMap(0);*/
    }
	...
	/*var imgExists = this.imgOcx.HS_OCX_LobIsExist(cardId, isTP, bty, fgp, vid, ABISCode.LobDataType.IMG, imgType, cid);
	 ...
	 var imgExists = this.imgOcx1.HS_OCX_LobIsExist(cardId, isTP, bty, fgp, vid, ABISCode.LobDataType.IMG, imgType, cid);
	 var mntExists = this.imgOcx1.HS_OCX_LobIsExist(cardId, isTP, bty, fgp, vid, ABISCode.LobDataType.MNT, mntId, cid);
	 if(imgExists === 0 && mntExists === 0)
	 {*/
    if(isTP === 1)
    {
        this.searchTP(cardId, bty, vid, fgp, cid, mntId, ABISCode.FGPDataType.ALL, imgType, type,isCand);
    }
    else
    {
        this.searchLP(cardId, bty, cid, ABISCode.FGPDataType.ALL, imgType, mntId, type,isCand);
    }
	/*}
	 ...
	 this.imgOcx.HS_OCX_DoDigitMap(0);
	 this.imgOcx1.HS_OCX_SelectImageMnt(cardId, isTP, bty, fgp, vid, imgType, mntId, cid);
	 this.imgOcx1.HS_OCX_DoDigitMap(0);
	 }*/
}
/*展示候选列表*/
FigureCheck.prototype.showTable = function(){
    var e = $('#DownArea');
...
/*弹出候选列表*/
FigureCheck.prototype.alertTable = function(){
    var e = $('#DownArea');
    e.show();
    var d = dialog({
        id: 'EF893L1',
...
/*弹出图像工具*/
var d;
FigureCheck.prototype.showTool = function(){
    if(d){
        d.open&&d.close();
        return false;
    }
