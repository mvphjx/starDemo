...
AddRmtServerCfg.prototype.init = function()
{
	...
	this.is_direct_link			=WebUI.createCombo("is_direct_linkId","is_direct_link",null,"is_direct_link_NameId",true,false,"RMT_SERVER|IS_DIRECT_LINK",null,this.requiredField);
	this.initMap(textArray,this.is_direct_link);
	this.is_direct_superior		=WebUI.createCombo("is_direct_superiorId","is_direct_superior",null,"is_direct_superior_NameId",true,false,"RMT_SERVER|IS_DIRECT_SUPERIOR",null,this.requiredField);
	...
	this.qry_rmt				= WebUI.createSearchCombo("qryRmtId", "qry_rmt", "",1, false, this.requiredField,"RMT_SERVER|QRY_RMT_ROLE");
	this.initMap(textArray,this.qry_rmt);
	this.qry_local				= WebUI.createSearchCombo("qryLocalId", "qry_local", "",1, false, this.requiredField,"RMT_SERVER|QRY_LOCAL_ROLE");
	this.initMap(textArray,this.qry_local);
...
AddRmtServerCfg.prototype.initComboData=function()
{
	var columnnames = ['RMT_SERVER|UNIT_CODE','RMT_SERVER|IS_DIRECT_LINK','RMT_SERVER|IS_DIRECT_SUPERIOR',
	                   'RMT_SERVER|IS_FRONT_SERVER','RMT_SERVER|QRY_RMT_ROLE','RMT_SERVER|QRY_LOCAL_ROLE'];	
...
AddRmtServerCfg.prototype.getComboData=function(data)
{
	...
	this.qry_local.setComboData(data['rmt_server|qry_local_role']);
	this.qry_rmt.setComboData(data['rmt_server|qry_rmt_role']);
}
...
AddRmtServerCfg.prototype.clear = function()
{
	this.qry_rmt.clear();
	this.qry_local.clear();
}
AddRmtServerCfg.prototype.setQryrmtVal = function(val)
{
	this.qry_rmt.setText(val);
}
AddRmtServerCfg.prototype.setQrylocalVal = function(val)
{
...
AddRmtServerCfg.prototype.setServer= function(server)
{
	...
	this.qry_local.setValue(this.formatBoolean(server.qry_local));		
	this.qry_rmt.setValue(this.formatBoolean(server.qry_rmt));
}
//转换 true = 1 false = 0
