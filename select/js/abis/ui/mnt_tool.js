...
var menuIdList_NoMousedown = ["extractMnt","clearMnt","ridgePattern"];

var menuIdList_MousedownLimit = [];//限制不让点击

var mntToolData  = 
[
	{bty:ABISCode.Bty.BTY_FINGER,	type:ABISCode.TableTypeCode.TPCARD,tool:[0,1,2,3,4,5,6,8,9,21,22,23,24,25,26]},
	{bty:ABISCode.Bty.BTY_PALM,		type:ABISCode.TableTypeCode.TPCARD,tool:[0,1,6,9,11,12,13,14,21,22]},
	{bty:ABISCode.Bty.BTY_FACE,		type:ABISCode.TableTypeCode.TPCARD,tool:[0,19]},
	{bty:ABISCode.Bty.BTY_FINGER,	type:ABISCode.TableTypeCode.LPCARD,tool:[0,1,2,3,4,5,6,8,9,10,15,21,22,23,24,25,26]},
	{bty:ABISCode.Bty.BTY_PALM,		type:ABISCode.TableTypeCode.LPCARD,tool:[0,1,6,8,9,10,11,12,13,14,16,17,18,21,22]},
...
function MntToolObject()
{
			...
			(
				function(e)
				{
					...
					if(obj.toolId != tId)
					{
						...
						var $this = $(this);
						if($.inArray(tId,menuIdList_NoMousedown)>-1){
							setTimeout(function (){
								$this.attr("class","mntbnt");
								obj.toolId[obj.bty] = mntTools[0];
							},500); 
						}
					}
				}
		...
		if(!WebUtil.isNull(ocx))
		{
			...
			switch(toolId)
			{
				...
				case "delete":
					...
					break;
				case "extractMnt":
					var gid = parseInt(this.getCardOptions().gid);
					var mid = parseInt(this.getCardOptions().mid);
					// ocx.HS_OCX_AutoExtractMnt(mid,gid,0);
                    ocx.HS_OCX_AutoExtractMntEx(mid,gid,0);
					return;
				case "clearMnt":
					ocx.HS_OCX_ClearAllMnt(0);
					return;	
				case "ridgePattern":
					tid = MntToolName.RIDGEPATTERN;
					break;
                case "markPoint":
                    tid = MntToolName.DATUMMARK;
                    break;
                case "unmatching":
                    tid = MntToolName.MISMATCHINGMNT;
                    break;
                case "matchmarkpoint":
                    tid = MntToolName.MATCHMARKPOINT;
                    break;
				default:return;
			}
	...
	MntToolObject.prototype.switchTool = function(type,bty)
	{
        ...
        {
        	return;//缺省不选中第一个  待完善 TODO  2017年8月24日16:45:59 hjx
	...
	//获取卡片信息  方法由父页面注入
	MntToolObject.prototype.setGetMsgFucntion = function(getCardOptions)
	{
		if(WebUtil.isFunction(getCardOptions)){
			this.getCardOptions = getCardOptions;//获取卡片信息
		}else{
