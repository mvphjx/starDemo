/**
	...
	参数格式:
	{
		...
		pageBarId	: "页码ID",
		callBack	: {onClick:selectItem,dbClick:function,refresh:function},
...
TableControlMgr.prototype.initTable = function()
{
	var setting = 
	{
		...
		checkAll	: this.param.checkAll==false?false:true,
		changeCheckStatusRegister:this.param.changeCheckStatusRegister
	};
...
/** 设置常规检索条件参数 */
TableControlMgr.prototype.setQryParam = function(where,orderCol,order)
{
	this.qryParam.where 	= where;
	this.qryParam.orderCol  =  orderCol||this.qryParam.orderCol;
	this.qryParam.order  =  order||this.qryParam.order;
	this.param.where		= where;
	this.param.orderCol  =  orderCol||this.param.orderCol;
	this.param.order  =  order||this.param.order;
	this.qryParam.distinct  =  this.param.distinct;
	this.qryParam.disColName  =  this.param.disColName;
	this.isQryCnt 			= true;
...
TableControlMgr.prototype.search = function(curPage,isQryCnt)
{
	...
	this.qryParam =
	{
		...
		qryCnt		: true,
		checkedIds	: this.param.checkedIds,
		checkedColName	: this.param.checkedColName||"ID",
		distinct	: this.param.distinct,
		disColName	: this.param.disColName
	};
	if(pageCnt == null)
	{
	...
	this.refresh(this.param.invoke);
}
 	...
 	jQuery.ajax(
		        {
					...
					{
						//当操作第一页以后的数据刷新页面
						if(data.pageCount!=0&&data.tblData.result.length==0){
                            nThis.qryParam.curPage = 1;
                            nThis.isQryCnt = true;
                            nThis.refresh();
						}
						...
						if(data.status == "ok"){
							...
							if(!WebUtil.isNull(data.pageCount))
							{
								...
								{
									nThis.pageBar = new WebTablePageNumber(nThis.param.pageBarId,pageClick,nThis.param.language,nThis.param.statuscolor,{"pageBntClass":nThis.param.pageBntClass});
									var n = Math.ceil(data.recordCount / nThis.qryParam.pageSize);
									var b = 1;
									while(n > 10)
									{
										n = n / 10;
										b++;//获取页数  最大有几位    例如  1001 BntWidth->44px ； 最小2位 24px
									}
									if(b == 1) b = 2;
									var width = (b * 10) + 4;
								...
								{
									if(true||(data.recordCount > nThis.qryParam.pageSize)) //如果大于设置的显示数才显示分页
									{
...
TableControlMgr.prototype.exportDialog = function(key)
{
	...
	//以前是导出选中的列，现在是导出页面全部列
	var rows = this.table.getAllItems();
	...
	$("input[name='ids']").val(idsStr);//当前列表的所有id
