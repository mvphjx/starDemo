...
var WebUI =
{
	...
	createDateTimeText:function(pId,id,cssName,title,colNameList,my97Options)
	{
		var webField = new WebInput(pId,id,cssName,this.WebInputType.DateTime,null,false,false,null,title,colNameList,null,null,null,my97Options);
		return webField;
	},
	createTimeText:function(pId,id,cssName,title,colNameList,my97Options)
	{
		var webField = new WebInput(pId,id,cssName,this.WebInputType.Time,null,false,false,null,title,colNameList,null,null,null,my97Options);
		return webField;
	},
	...
	createMultipleCombo:function(pId,id,cssName,combotype,flag,issearchall,colNameList,colName,itemtype,chosenOptions)
	{
		var webField = new WebInput(pId,id,cssName,this.WebInputType.MultipleCombo,null,flag,false,colName,null,colNameList,combotype,itemtype,null,null,chosenOptions);
		return webField;
	},
 ...
 * @param my97Options	data(json)  my97的配置数据
 * @param chosenOptions  data(json)  chosen的配置数据
 */
function WebInput(pId,id,css,type,rId,flag,issearchall,columnName,title,colNameList,combotype,itemtype,placeType,my97Options,chosenOptions)
{
	this.id 		= id;
	...
	var tWidget = null;
	if(WebUtil.isNull(type) || type == WebUI.WebInputType.Text || type == WebUI.WebInputType.Date ||  type == WebUI.WebInputType.DateTime ||  type == WebUI.WebInputType.Time)
	{
		...
		{
			var settting  = {maxDate:"%y-%M-%d"};
			if(!WebUtil.isEmpty(my97Options)){
				if(my97Options.NoTimeLimit){
					settting = {};
				}else{
		...
		}else if(type == WebUI.WebInputType.Time) {
			var settting  ={
					dateFmt : 'HHmmss'
					};
			if(!WebUtil.isEmpty(my97Options)){
		...
		//this.keyup();//registerTextListener 已经实现校验  这个待优化
	}
	...
	{
		this.multipleCombo = new WebMultipleCombo(id,placeType,flag, issearchall,columnName,0,clickInvoke, null, null,itemtype,chosenOptions) ;
		var nThis = this;
		function clickInvoke(value,checked)
		{//TODO
			return;
			if(!WebUtil.isNull(nThis.listener))
			{
	...
	function registerTextListener(id,text)
	{
		...
		(
			function(e)
			{
				...
				if(nThis.prevValue == $(this).val())return;
				nThis.prevValue = $(this).val();
				if(WebUtil.isFunction(nThis.listener))
				{
...
{
	if(this.type == WebUI.WebInputType.Text || this.type == WebUI.WebInputType.Date|| this.type == WebUI.WebInputType.DateTime|| this.type == WebUI.WebInputType.Time)
	{
		this.text.focus();
	}
...
WebInput.prototype.cancelErrorTip = function()
{
	...
	else if(this.type == WebUI.WebInputType.Password)
	{
		this.text.removeClass("Error_Field_Tip").removeClass("Required_Input_Tip").addClass("Text_Input");
	}
	else
	...
	if(v != null && v != "" && this.validateParam != null)
	{
		if(this.validateParam.maxlength != null)
		{
			...
			if(vl > ml)
			{
				...
				{
					this.infotip.html(AbisMessageResource.Alert["CanBeNoLongerThan"]+ml+AbisMessageResource.Character);
				}
				return false;
			}
		}
		if(this.validateParam.minlength!=null)
		{
			...
			if(vl<ml)
			{
				...
				if(this.infotip != null)
				{
					this.infotip.html(AbisMessageResource.Alert["CanNotBeLessThan"]+ml+AbisMessageResource.Character);
				}
				return false;
			}
		}
		...
		if(this.validateParam.mixvalue != null||(this.validateParam.minvalue != null))
		{
			var t= WebUtil.isNumber(v);
			if(t)
			{
				var ml = this.validateParam.mixvalue||this.validateParam.minvalue;
				if(v < ml){
					this.errorTip();
					if(this.infotip != null)
					{
...
WebInput.prototype.setEditable = function(e)
{
	...
	if(this.type == WebUI.WebInputType.Text || this.type == WebUI.WebInputType.DateTime|| this.type == WebUI.WebInputType.Date|| this.type == WebUI.WebInputType.Time)
	{
...
{
	if(this.type == WebUI.WebInputType.Text || this.type == WebUI.WebInputType.DateTime|| this.type == WebUI.WebInputType.Time)
	{
		return this.text;
	}
	else if(this.type == WebUI.WebInputType.MultiText)
...
WebInput.prototype.getText = function()
{
	...
	if(this.type == WebUI.WebInputType.Text || this.type == WebUI.WebInputType.Date||this.type==WebUI.WebInputType.Password|| this.type == WebUI.WebInputType.DateTime|| this.type == WebUI.WebInputType.Time)
	{
		value =  this.text.val();
	}
	else if(this.type == WebUI.WebInputType.MultiText)
...
WebInput.prototype.clear = function()
{
	...
	else if(this.type == WebUI.WebInputType.Text || this.type == WebUI.WebInputType.Date|| this.type == WebUI.WebInputType.DateTime|| this.type == WebUI.WebInputType.Time)
	{
		this.text.val("");
	}
	else if(this.type == WebUI.WebInputType.MultiText)
...
WebInput.prototype.setValue = function(value,notify)
{
	...
	if(this.type == WebUI.WebInputType.Text  || this.type == WebUI.WebInputType.Date  || this.type == WebUI.WebInputType.DateTime  || this.type == WebUI.WebInputType.Password|| this.type == WebUI.WebInputType.Time)
	{
		this.text.val(value);
	}
	else if(this.type == WebUI.WebInputType.Label)
...
//监控要操作的控件
function webInputCascadingListener(activeWebInput,readonlyWebInput,url,monitoredObj,param) 
{
	if(activeWebInput!=""){
		activeWebInput.addChangeListener(
			function(id,value,txt){
				...
				$("#"+monitoredObj+"code").val("");
				//清除上一次操作下拉选的值
				$("#"+monitoredObj+"list").html("");
				readonlyWebInput.setEditable(true);
				$.ajax
				(
			        {
						type 		: 'POST',
						...
						{
							readonlyWebInput.setComboData(eval('(' + data + ')'));
						},
						error : function(e)
						{
							WebUtil.NoWait();
