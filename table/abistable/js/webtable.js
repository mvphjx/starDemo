 ...
 *            	pageBarId :分页ID
 *            	isPopup		: true 弹窗中的表格，false 非弹窗中的表格
 *            }
 * 注意区分   rowIndex->dom
 * 		    objIndex ->data
 */
...
WebTable.prototype.init = function() {
    ...
    /* 冻结列表格 */
	this.freezeDivId = this.id+"_freezeTable";
    this.$freezeDiv = $("<div style=\" float: left; \" class=\"freezeCols\" id=\""+this.freezeDivId+"\"></div>");

   this.freezeTableId = this.id+"_freezeTable_table";
    this.$freezeTable = $("<table id=\""+this.freezeTableId+"\" cellspacing=\"0\" cellpadding=\"0\"" +
		" class=\"gtable\"></table>");
	this.$freezeDiv.append(this.$freezeTable);
	this.parent.append(this.$freezeDiv);

	/* 创建表格 */
    this.$tableDiv = $("<div style=\" width: 100%;height:100%; float: left;\"></div>");
	...
	this.parent.append(this.$tableDiv);
 ...
 * 冻结列相关样式初始化，外界禁止调用
 */
WebTable.prototype.initFreezeTable = function () {
    var $ftable = $("#"+this.freezeTableId);
    var $ftbody = $ftable.find("tbody");

	if(!WebUtil.isEmpty($ftbody.html()))
	{
        this.$tableDiv.css("width","100%");
	}

	if(!this.setting.isPopup&&!WebUtil.isEmpty(this.setting)&&!WebUtil.isEmpty(this.setting.freezeCols))
	{
		//修改原有table和上级的上级div滚动条
        this.$tableDiv.css("overflow","hidden");
	}
 ...
 * 冻结列赋值，外界禁止调用
 */
WebTable.prototype.setFreezeTableData = function(){
	// if (!WebUtil.isEmpty(this.setting) && !WebUtil.isEmpty(this.setting.freezeCols)){
    if(!WebUtil.isEmpty(this.setting)&&!WebUtil.isEmpty(this.setting.freezeCols)){
    	//修改原有table和上级的上级div滚动条
        this.$tableDiv.css("overflow","auto");
        this.parent.parent().css("overflow","hidden");
        //填充冻结列
		var $ftable = $("#"+this.freezeTableId);
		$ftable.find("tbody").remove();//清空冻结列表格
		var $ftbody = $("<tbody></tbody>");
		...
		$table.find("tbody").find("tr").each(function () {
            ...
            var cols = nThis.setting.freezeCols;
            var len =cols.length;
			if(!$this.hasClass("header")){
				var $tr = $("<tr></tr>");
				$tr.attr("id",$this.attr("id").replace("_table_","_freezeTable_"));
				$tr.attr("class",rowClass);


				 for(var i=0; i<len;i++){
                     var col = cols[i];
                     var index = $.inArray(col, nThis.input.header);

                     var $ftd = $("<td></td>");
                     var $td = $this.find("td:eq("+index+")");
                     $ftd.attr("class",$td.attr("class"));
                     $ftd.css("border-right","1px solid #e4e5e6;");
                     $td.hide();

                     if($td.find("a").length){
                         var $link = $($td.html());
                         $ftd.append($link);
                         nThis.registerLink($link);
                     }else{
                         $ftd.html($td.html());
                     }
                     $tr.append($ftd);
                ...
                nThis.registerRow($tr, nThis.setting);

			}else{
				var $tr = $("<tr></tr>");
				$tr.attr("class",rowClass);
                var freezeDivWidth = 0;
                var freezeMinWidth = 0;
                for(var i=0; i<len;i++){
                    var col = cols[i];
                    var index = $.inArray(col, nThis.input.header);
                    var $ftd = $("<th></th>");
                    var $td = $($this.find("th:eq("+index+")"));

                    $ftd.attr("class",$td.attr("class"));
                    $ftd.css("border-right","1px solid #e4e5e6;");
                    $ftd.attr("index", $td.attr("index"));
                    $ftd.attr("id", "freezeCol"+$td.attr("id"));
                    $ftd.html($td.html());
                    $tr.append($ftd);
                    freezeDivWidth += $td.outerWidth()+2;

                    //$td.hide();
                    nThis.registerHeader($ftd);
                }

				$("#"+nThis.freezeDivId).css("width",freezeDivWidth);
				$("#"+nThis.tableId).parent().css("width",$("#"+nThis.tableId).parent().parent().outerWidth(true)-freezeDivWidth-1);
...
WebTable.prototype.addRowData = function(row, index) {
	...
	if(!WebUtil.isEmpty(this.setting.freezeCols)){//设置冻结列，冻结列表格增加行
		var $freezeTable = $("#" + this.freezeTableId);
		$freezeTable.append($freezeRow);
		this.registerRow($freezeRow);
	}
	...
	for ( var j = 0; j < header.length; j++) {
		...
		//冻结列处理 隐藏原有列，复制原有列到冻结列里
		if(!WebUtil.isEmpty(this.setting)&&!WebUtil.isEmpty(this.setting.freezeCols)&&$.inArray(colName, this.setting.freezeCols)>=0){
            var $ftd = $("<td></td>");
            $ftd.attr("class",$td.attr("class"));
            if (this.isLinkCol(colName)) {
                var $flink = $($td.html());
                $ftd.append($flink);
                $td.hide();
                this.registerLink($flink);
            }
            $freezeRow.append($ftd);
		}
        $row.append($td);
    }
    //禁止选中内容
    this.forbiddenSelectContent();
	return true;
}
...
WebTable.prototype.checkData = function() {
	...
	if(WebUtil.isEmpty(this.input.result)){//刷新
        this.initTableHead();
    }
}
 ...
 *            {data:{dbid:"1:捺印库(1)",cardId:1001,cardNum:"110108201205060001",name:"张海峰",sex:"男"},extdata:根据业务扩展的属性},
 *            {data:{"捺印库(1)",1021,"110108201206070001","张兰","女"}, "readonly":false,"isCheck": false}] };
 */
...
WebTable.prototype.updatePageCoder = function() {
	...
	if (WebUtil.isNull(this.input) || WebUtil.isEmpty(this.input.result)) {
		...
		var hideCount = this.input&&this.input.hideColList&&this.input.hideColList.length||0;
		for ( var i = 0; i < colCount-hideCount; i++) {
			$td = $("<td class=\"cell first_cell\"></td>");
			$row.append($td);
		}
        if(!WebUtil.isEmpty(this.setting)&&!WebUtil.isEmpty(this.setting.freezeCols)){
            var $frow = $("<tr class=\"" + cssName + "\"></tr>")
            var cols = this.setting.freezeCols;
            var $this = $(this);
            for(var i=0,len =cols.length; i<len;i++){
                var col = cols[i];
                var index = $.inArray(col, this.input.header);

                var $ftd = $("<td class=\"cell\"></td>");
                var $td = $this.find("td:eq("+index+")");
                $ftd.attr("class",$td.attr("class"));
                $ftd.css("border-right","1px solid #e4e5e6;");
                $td.hide();
            ...
            $("#"+this.freezeDivId+"_table").append($frow);
		}
	} else {
...
WebTable.prototype.fillData = function() {
	...
	for (i = 0; i < this.indexList.length; i++) {
        ...
        var styleRow = WebUtil.string2Json(res[index].styleRow);//行样式
        var style = WebUtil.string2Json(res[index].style);//列样式
        $row.removeAttr("style");
        if(!WebUtil.isEmpty(styleRow)){
            for(var key in styleRow){
                $row.css(key,styleRow[key]);
            }
        }
        for (j = 0; j < colCount; j++) {
			...
			/*字段过长截取*/
			if(content!=null&&WebUtil.getLength(content)>WebTable.CONTENT_LENGTH){
				content_td=content;
				content=WebUtil.getSubstr(content,WebTable.CONTENT_LENGTH-4)+"...";
				$td.attr("content",content_td);
			}
			/*字段过长截取 end*/
			$td.text(content);
            $td.removeAttr("style");
            if(!WebUtil.isEmpty(style)){
                for(var key in style[colName]){
                    $td.css(key,style[colName][key]);
                }
            }
            var hideColList = this.input.hideColList;
            if(!WebUtil.isEmpty(hideColList)){//隐藏列
                for(var iHide=0 ;iHide<hideColList.length;iHide++){
                    var hideColName = hideColList[iHide];
                    if(hideColName===colName){
                        $td.hide();
                        break;
                    }
                }
 ...
 * 清空td tr的样式
 */
WebTable.prototype.clearCss = function(){
...
WebTable.prototype.initTable = function() {
	...
	for ( var i = 0; i < len; i++) {
		...
		$row.attr("id", rowId);
		$row.attr("cssName", cssName);
        var styleRow = WebUtil.string2Json(res[index].styleRow);
		for(var key in styleRow){
            $row.css(key,styleRow[key]);
        }
        var style = WebUtil.string2Json(res[index].style);//列样式
        ...
        var readonly = $rowData.readonly == true ? true : false;
		if (isCheck) {
			...
			// var checked = $rowData.isCheck == true ? "checked" : "";
			checkBox.attr("checked", checked);
            checkBox.attr("disabled", readonly);
            checkBox.attr("rowId", rowId);
			td.append(checkBox);
			$row.append(td);
		...
		for ( var j = 0; j < colCount; j++) {
			...
			if(content_td!=null){
				...
				if(typeof td_showdiv =="undefined " || td_showdiv==null){
					...
					$(document).on('mouseenter','td.cell_min_w',function(event){
			            $thiss=$(this);
			            var content_str=$thiss.attr("content")
			        ...
			        }).on('mouseleave','td.cell_min_w',function(){
...
获取表格 行列样式
 ...
 status==="1"  标示列配置
 */
WebTable.prototype.getStyle = function (columnName,value) {
...
WebTable.prototype.registerRow = function($row,setting) {
	...
	$row.mouseover(function() {
		...
		var _rowItems = rowId.split("_");
		var objIndex = _rowItems[_rowItems.length-1];

		if ($(this).attr("class") != "rowdown") {
            $("#"+nThis.tableId + "_" + objIndex).addClass("rowhover");
            $("#"+nThis.freezeDivId + "_" + objIndex).addClass("rowhover");
		}
	});
	$row.mouseout(function() {
        ...
        if ($(this).attr("class") != "rowdown") {
            $("#"+nThis.tableId + "_" + objIndex).removeClass("rowhover");
            $("#"+nThis.freezeDivId + "_" + objIndex).removeClass("rowhover");
        }
	});
 ...
 * 禁止选中文本
 */
WebTable.prototype.forbiddenSelectContent = function () {
	//禁止选中事件
	this.forbiddenSelect(this.tableId);
	this.forbiddenSelect(this.freezeTableId);

    // var nThis = this;
    // //双击可选中
    // $("#" + this.tableId).find("tr td").bind("dblclick", function(e) {
    //     var $this = $(this);
    //     nThis.enableSelect($this, nThis);
    // });
}
 ...
 * @param tableid
 */
WebTable.prototype.forbiddenSelect = function (tableid) {
    var _this = this;
    $("#" + tableid).find("tr").bind("selectstart", function(e) {
        return !_this.isShiftKey;//只屏蔽shift选中功能。鼠标选中功能正常使用。
    });
}

/**
 * 选中单元格内容
 * @param $tdObj 单元格jquery对象
 * @param nThis WebTable对象
 */
WebTable.prototype.enableSelect = function ($tdObj,nThis) {
    nThis.forbiddenSelect(nThis.tableId);
    nThis.forbiddenSelect(nThis.freezeTableId);
    $($tdObj.parent("tr")[0]).unbind("selectstart");
    $tdObj.select();
}
/**
 * 单行选择(外界禁止调用)
...
WebTable.prototype.selectRow = function($row, objIndex) {
    ...
    $("#"+this.tableId + "_" + objIndex).addClass("rowdown");
    $("#"+this.freezeDivId + "_" + objIndex).addClass("rowdown");
...
WebTable.prototype.deSelectRow = function(rowIndex) {
	...
	var $row = $("#"+this.tableId + "_" + objIndex);
	var cssName = $row.attr("cssName");
	$row.attr("class", cssName);
	$row.removeClass("rowdown");
    $("#"+this.freezeDivId + "_" + objIndex).removeClass("rowdown");
}
...
WebTable.prototype.deSelectObj = function(objIndex) {
    ...
    $("#"+this.tableId + "_" + objIndex).removeClass("rowdown");
    $("#"+this.freezeDivId + "_" + objIndex).removeClass("rowdown");
}
 ...
 * 获取表格记录数
 */
WebTable.prototype.getSize = function(){
...
WebTable.prototype.deleteRow = function(index) {
	...
	//冻结表格删除行
    $("#" + this.id + "_freezeTable_" + index).remove();
}
 ...
 * rowIndex 数据的索引
 */
WebTable.prototype.getRowDataPlus = function(rowIndex) {
	if (rowIndex == null)
		return null;
	//var objIndex = this.indexList[index];
	var data1 =$.extend(true, {}, this.getRowByObjIndex(rowIndex));
	var data2 =this.input.result[rowIndex].data;
	for ( var j = 0; j < this.input.header.length; j++) {
 ...
 *  return  code
 */
WebTable.prototype.getRowData = function(rowIndex) {
	if (rowIndex == null)
		return null;
	//var objIndex = this.indexList[index];
	var data1 =$.extend(true, {}, this.getRowByObjIndex(rowIndex));
...
WebTable.prototype.setSelectObjRow = function(n) {
    ...
    var freezeRowIndex = $row.attr("id").replace("_table_","_freezeTable_");
    $("#"+freezeRowIndex).addClass("rowdown");
}
...
WebTable.prototype.getShowCols = function() {
	return WebUtil.removeAll(this.input.header,this.input.hideColList);
}
WebTable.prototype.getHideCols = function() {
    return this.input.hideColList;
}
...
/*勾选 指定数据*/
WebTable.prototype.checkRows= function(checkIndexs) {
    if (this.input.result == null)
        return null;
    var indices = new Array();
    if(WebUtil.isNumber(checkIndexs)){
        checkIndexs  =  [checkIndexs];
	}
    for ( var i = 0; i < this.indexList.length; i++) {
        if ($.inArray(i+1, checkIndexs) > -1) {
            var idx = this.indexList[i];
            var row = this.input.result[idx];
            row.isCheck=true;
            var $table = $("#" + this.tableId);
...
WebTable.prototype.deCheckAllRows= function() {
	...
	for ( var i = 0; i < this.indexList.length; i++) {
		...
		var $tBody = $table.find("TBODY");
		var $tr = $tBody.find("tr:eq(" + (i+1) + ")");
        var disabled = $tr.find('input').first().attr("disabled")
