WebTablePageNumber.prototype.pageCoderBntCnt 	= 7;//翻页按钮数量
WebTablePageNumber.prototype.pageCoderBntWidth 	= 24;//翻页按钮宽度
WebTablePageNumber.prototype.pageLimit = 100;//最大允许翻页数
WebTablePageNumber.prototype.pageSizeOptions = [30,50,100,200,500];//每页记录数可选项
/**
 * 分页控件
 * @param pid  dom id
 * @param invoke  回调事件  function(pid,curPage) //dom id，当前页数
 * @param language 国际化语言参数
 * @param isBg boolean 是否显示背景色
 * @param param  { pageBntClass:分页样式 。不传默认为普通分页样式，传值"simplePage"简化版分页样式,
 * 					selPageSize:是否可选每页记录数 true 可选  false 不可选 默认不可选
 * 					selPageSizeInvoke:选择每页记录数的回调函数
 * 					}
 * @constructor
 */
function WebTablePageNumber(pid, invoke, language, isBg, param)
{
	...
	this.visible	= true;
	this.param		= param;
	//this.param.pageBntClass 分页样式 ：不传默认为普通分页样式，传值"simplePage"简化版分页样式

	if(WebUtil.isNull(this.param)){
		this.param={};
	}
	if(typeof this.param.pageBntClass =="undefined")
	{
		this.param.pageBntClass = "bnt";
	}
	if(typeof this.param.selPageSize =="undefined")
	{
		this.param.selPageSize = false;
	}
...
WebTablePageNumber.prototype.init = function()
{
	...
	if(this.param == null||typeof this.param.showResCnt == "undefined" || this.param.showResCnt == true)
	{
...
WebTablePageNumber.prototype.setCurPage = function(page)
{
...
WebTablePageNumber.prototype.setPageCnt = function(pageCnt, pageSize)
{
    ...
    if(this.curPage == undefined || this.curPage == "undefined" || this.curPage == null){
        this.curPage = 1;
	}
	//首页
	var firstPageTxt 	= "";
	...
	var $firstPage = $("<div  class=\""+this.param.pageBntClass+"_firstPage PageCodeBnt CtrlBntSize BntNormal \">"+ firstPageTxt +"</div>");
	$firstPage.attr("type", "first");
	this.registerButton($firstPage);
	this.pageBar.append($firstPage);

	//前一页
	var $prevPage = $("<div  class=\""+this.param.pageBntClass+"_prevPage PageCodeBnt CtrlBntSize BntNormal \">" + prevPageTxt + "</div>");
    ...
    var start = this.curPage - Math.floor(offset);
    var end = this.curPage + Math.ceil(offset);
    //越界判断
    if (start < 1)
    {
        start = 1;
    ...
    for ( var i = start; i <= end; i++){
        $pageCode = $("<div  class=\""+this.param.pageBntClass+"_pageCode PageCodeBnt PagingBntSize \"></div>");
        if (i == this.curPage)
        {
        ...
        $pageCode.html(i);
        $pageCode.attr("id", this.pid + "_" + (i-start+1));
	...
	var $nextPage = $("<div  class=\""+this.param.pageBntClass+"_nextPage PageCodeBnt CtrlBntSize BntNormal \">" + nextPageTxt + "</div>");
	$nextPage.attr("type", "next");
	this.registerButton($nextPage);
	this.pageBar.append($nextPage);

	var $lastPage = $("<div  class=\""+this.param.pageBntClass+"_lastPage PageCodeBnt CtrlBntSize BntNormal \">" + lastPageTxt + "</div>");
	$lastPage.attr("type", "last");
	this.registerButton($lastPage);
	this.pageBar.append($lastPage);

	var $pageCount = $("<div class=\""+this.param.pageBntClass+"_pageCount PageCntLab \"></div>");
	...
	var $turnTxt = $("<input type=\"text\" class=\""+this.param.pageBntClass+"_turnTxt TurnTxt \"/>");
	...
	//页码标签
	var $Lablepage = $("<div class=\""+this.param.pageBntClass+"_labelPage PageCntLab \"></div>");
	$Lablepage.html(" "+pageTxt+" ");
	this.pageBar.append($Lablepage);		
	
	var $pageBnt = $("<div class=\""+this.param.pageBntClass+"_toPage PageBnt ToPageBntWidth \">"+ pagejump +"</div>");
    this.registerPageNumBnt($pageBnt);
    this.pageBar.append($pageBnt);
    var nThis = this;
	if(this.param.selPageSize)
	{
        //每页条数
		var $pageSizeOpt = $("<div class=\""+this.param.pageBntClass+"_pageSizeOptions pageSizeOptions\"" +
			" style='display: none;'></div>");
		var $pageSizeOptUl = $("<ul></ul>");
		for(var i=0,arrLen = this.pageSizeOptions.length;i<arrLen;i++)
		{
			var o = this.pageSizeOptions[i];
			var $li = $("<li>"+o+"</li>");
			if(o==pageSize)
			{
                $li.addClass("select");
            ...
            $li.bind("click",function () {
                $(".PageSize").find(".select").removeClass("select");
                var $thisLi = $(this);
                $thisLi.addClass("select");
                nThis.selectPageSize(nThis,$thisLi.html());//选择页面大小后回调函数
            });
            $pageSizeOptUl.append($li);
		}
    	$pageSizeOpt.append($pageSizeOptUl);
        var $pageSize = $("<div class=\""+this.param.pageBntClass+"_pageSize PageSize \"></div>");
        $pageSize.html("<div class='val'>"+pageSize+"</div>");
        $pageSize.find(".val").bind("click",function () {
            $pageSizeOpt.css("display","block");
        });
        $pageSize.bind("mouseleave",function () {
            $pageSizeOpt.css("display","none");
        });
    	$pageSize.append($pageSizeOpt);
        this.pageBar.append($pageSize);
        var $pageSizeTxt = $("<div class=\""+this.param.pageBntClass+"_PageCntLab PageCntLab \"></div>");
        $pageSizeTxt.html( recordsPageTxt);
        this.pageBar.append($pageSizeTxt);
	}
	else {
        var $pageSizeTxt = $("<div class=\""+this.param.pageBntClass+"_PageCntLab PageCntLab \"></div>");
        $pageSizeTxt.html( pageSize+recordsPageTxt);
        this.pageBar.append($pageSizeTxt);
	}

}
WebTablePageNumber.prototype.selectPageSize = function (nThis,pageSize) {
    $("." + nThis.param.pageBntClass + "_pageSize").find(".val").html(pageSize);
    $("." + this.param.pageBntClass+"_pageSizeOptions").css("display","none");
    if(!WebUtil.isNull(nThis.param)&&nThis.param.selPageSize&&WebUtil.isFunction(nThis.param.selPageSizeInvoke))
	{
        nThis.param.selPageSizeInvoke(pageSize);
	}
}
 ...
 * @param pageNum 页码
 */
WebTablePageNumber.prototype.forwardPage = function(pageNum)
{
	var txtId = this.pid + "_TurnTxt";
    if (WebUtil.isEmpty(pageNum)) {
        pageNum = $("#" + txtId).val();
...
WebTablePageNumber.prototype.choosePage = function($bnt,isInvoke)
{
	...
	if (this.pageCnt > this.pageCoderBntCnt)
	{
		...
		if(this.curPage>this.pageLimit){
			var str =AbisMessageResource.Alert['PageToMore'].replace('{{page}}',this.pageLimit)
			setTimeout(function(){
				if(typeof DialogUtil =='undefined'){
					alert(str);
				}else{
					DialogUtil.openSimpleDialog(str);
				}}
				,0);
			this.curPage=curPageBak;
 			return false;
		}
